...
movie_titles = pd.read_csv('movieId.csv')
...
def viz_page():
    ...
    """
    with open("movie_rec_with_poster.html", 'r') as viz_file:
...
def rec_title():
    ...
    t = data["title"][0]
    scale = data["scale"]
    movie_ind = movie_titles.index[movie_titles['title'].str.lower()==t.lower()]
    rec_ind = similar_movie_by_title(movie_ind, nmf_topic_vec_normalized, genre_vec, scale)
    rec_movies = movie_titles['title'][rec_ind]
    # rec_movie_id = movie_titles['movieId'][rec_ind]
    poster_href = ['static/poster/'+str(mid)+'.jpg' for mid in movie_titles['movieId'][rec_ind]]
    # results = {"movies": list(rec_movies), "poster": poster_href}
    results = {"movies": list(zip(poster_href, rec_movies))}
